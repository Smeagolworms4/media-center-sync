...
| `override-dialog`, `override-form`, `override-save`, `override-restore`, `override-reset` | `OverrideDialog.vue` | correcting what a media server got wrong, putting it all back at once, and the other half of that decision: filling the boxes with what the service reports so it can be read before anybody saves. Saving after a reset sends an empty body, which removes the correction rather than storing one identical to the service's answer — the difference between an item that goes on following its server and one frozen on today's values |
...
| `pagination`, `pagination-range`, `pagination-limit`, `pagination-first`, `pagination-previous`, `pagination-next`, `pagination-last` | `Pagination.vue` | the pager shared by every paginated screen. Marked because paging is one of the few things whose defect is only visible from a browser: the address bar, the request and the rows have to agree, and nothing but a real click proves they do |
...
| `library-hints`, `library-hint`, `library-hint-text`, `library-hint-examples`, `library-hint-fix`, `library-hint-dismiss` | `LibraryHints.vue` | the two lines about how this gateway is organised, on the dashboard and the wall alike; a row carries `data-kind` — `nothing_mounted` or `misread_folder` — which is how a journey addresses the one it is about, and only the folder one has a dismissal |
