...
| `library-path-browse`, `service-local-root-browse` | `LibraryPathField.vue`, `ServiceForm.vue` | the icons that open the directory picker on a library path and on a service's local root |
| `browse-server`, `browse-server-hint`, `browse-server-entry`, `browse-server-enter`, `browse-server-up`, `browse-server-no-deeper`, `browse-server-unsupported`, `browse-server-empty`, `browse-gateway-title` | `DirectoryPicker.vue` | the server's half of the picker: its folders, the sentence relating them to this disk, the step in and back out, and the note kept beside the folders when the server cannot walk deeper |
