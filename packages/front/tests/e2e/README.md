...
| `settings-public-url`, `settings-public-url-suggested`, `settings-peer-address`, `settings-default-target` | `Settings.vue` | where this gateway is reached from outside, where peers connect, and where a pull lands when nothing else decides — the caption appears only while the address is still the browser's own origin and nothing has been stored, which is what makes it an offer rather than an assumption |
